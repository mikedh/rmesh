...
/// Load a mesh from raw bytes with an optional resolver for external references.
///
/// If `resolver` is `None`, external references (MTL files, textures) are silently skipped.
/// If `resolver` is `Some`, it will be used to resolve external file paths.
pub fn load_mesh(
    file_data: &[u8],
    ...
    match file_type {
        ...
        MeshFormat::OBJ => {
            ...
            ObjMesh::from_string(text.as_ref(), resolver).into_mesh()
        }
        MeshFormat::PLY => todo!(),
    }
}
