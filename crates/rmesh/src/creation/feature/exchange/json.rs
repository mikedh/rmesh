...
use super::super::{FeatureError, FeatureModel, Operation, Result};
...
pub fn write_json(path: impl AsRef<Path>, model: &FeatureModel) -> Result<()> {
    ...
    std::fs::write(path, content)
        .map_err(|e| FeatureError::ParseError(format!("Failed to write file: {}", e)))?;
    Ok(())
}
...
pub fn from_json(json: &str) -> Result<FeatureModel> {
    serde_json::from_str(json)
        .map_err(|e| FeatureError::ParseError(format!("JSON parse error: {}", e)))
}
...
pub fn to_json(model: &FeatureModel) -> Result<String> {
    serde_json::to_string_pretty(model)
        .map_err(|e| FeatureError::ParseError(format!("JSON serialize error: {}", e)))
}
...
mod tests {
    ...
    fn test_roundtrip_compact() {
        let model = FeatureModel::new().with_operation(Extrude::simple(Sketch::circle(5.0), 10.0));
    ...
    fn test_from_json_bytes() {
        let model =
            FeatureModel::new().with_operation(Extrude::simple(Sketch::rectangle(5.0, 5.0), 2.0));
