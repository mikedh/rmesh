                ...
                match e.name().as_ref() {
                    ...
                    b"xs:element" | b"xsd:element" => {
                        if in_sequence || in_choice {
                            ...
                            if let Ok(name_attr) = e.try_get_attribute("name") {
                                if let Some(name_bytes) = name_attr {
                                    field_name = String::from_utf8_lossy(&name_bytes.value)
                                        .to_case(Case::Snake);
                                }
                            }
                            ...
                            if let Ok(type_attr) = e.try_get_attribute("type") {
                                if let Some(type_bytes) = type_attr {
                            ...
                            if let Ok(min_occurs) = e.try_get_attribute("minOccurs") {
                                if let Some(min_bytes) = min_occurs {
                                    if String::from_utf8_lossy(&min_bytes.value) == "0" {
                                        is_optional = true;
                                    }
                                }
                            }
                    ...
                    b"xs:attribute" | b"xsd:attribute" => {
                        ...
                        if let Ok(type_attr) = e.try_get_attribute("type") {
                            if let Some(type_bytes) = type_attr {
                                attr_type = map_xsd_type_to_rust(&String::from_utf8_lossy(
                                    &type_bytes.value,
                                ));
                            }
                        }
                        ...
                        if let Ok(use_attr) = e.try_get_attribute("use") {
                            if let Some(use_bytes) = use_attr {
                                if String::from_utf8_lossy(&use_bytes.value) == "required" {
                                    is_optional = false;
                                }
                            }
                        }
...
fn map_xsd_type_to_rust(xsd_type: &str) -> String {
    match xsd_type {
        // Basic XSD types
        "xs:string" | "xsd:string" => "String".to_string(),
        "xs:int" | "xsd:int" => "i32".to_string(),
        "xs:integer" | "xsd:integer" => "i64".to_string(),
        "xs:long" | "xsd:long" => "i64".to_string(),
        ...
        "xs:double" | "xsd:double" => "f64".to_string(),
        "xs:decimal" | "xsd:decimal" => "f64".to_string(),
        "xs:boolean" | "xsd:boolean" => "bool".to_string(),
        "xs:dateTime" | "xsd:dateTime" => "String".to_string(), // Could use chrono::DateTime
        "xs:date" | "xsd:date" => "String".to_string(),
        "xs:time" | "xsd:time" => "String".to_string(),
        "xs:anyURI" | "xsd:anyURI" => "String".to_string(),
        "xs:ID" | "xsd:ID" => "String".to_string(),
        "xs:IDREF" | "xsd:IDREF" => "String".to_string(),
        "xs:token" | "xsd:token" => "String".to_string(),
        ...
        "ListOfTokens" => "Vec<String>".to_string(),
        "ListOfNames" => "Vec<String>".to_string(),
        "SidType" => "String".to_string(),
