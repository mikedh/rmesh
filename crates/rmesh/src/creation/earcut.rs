//! Vendored from [earcut-rs](https://github.com/ciscorn/earcut-rs) v0.4.4 (ISC license).
//!
//! Changes from upstream:
//! - Removed `no_std`, `utils3d`, `deviation`, `Index` impls for u16/u32
//! - Fixed `z_order` to saturate instead of panicking on NaN/negative/overflow
//! - Replaced remaining `unwrap()` calls with safe alternatives

// Vendored code uses unsafe for performance-critical node access (bounds-checked in debug).
#![allow(unsafe_code)]
...
impl<T: Float> Earcut<T> {
    ...
    fn reset(&mut self, capacity: usize) {
        ...
        self.nodes
            .push(Node::new(0, [T::infinity(), T::infinity()]));
    }
    ...
    fn earcut_impl(&mut self, hole_indices: &[usize], triangles_out: &mut Vec<usize>) {
        ...
        if self.data.len() > 80 {
            ...
            if inv_size != T::zero() {
                inv_size = T::from(32767.0).unwrap_or(T::zero()) / inv_size;
            }
        }
...
fn earcut_linked<T: Float>(
    ...
    loop {
        ...
        if is_ear {
            ...
            triangles.push(prev.i as usize);
            triangles.push(ear.i as usize);
            triangles.push(next_i as usize);
...
fn cure_local_intersections<T: Float>(
    ...
    loop {
        ...
        {
            triangles.extend([a.i as usize, p.i as usize, b.i as usize]);
...
fn index_curve<T: Float>(
    ...
    // Safe: prev_z_i was set to Some in the loop above for every node including this one.
    if let Some(p_prev_z_i) = p.prev_z_i.take() {
...
fn sort_linked<T: Float>(nodes: &mut [Node<T>], list_i: NodeIndex) {
    ...
    loop {
        ...
        // Safe: the while loop above always runs at least once, setting tail_i.
        if let Some(ti) = tail_i {
            node_mut!(nodes, ti).next_z_i = None;
        }
        if num_merges <= 1 {
            break;
        }
...
///
/// Saturates to 0 on NaN, negative, or overflow instead of panicking.
