use super::indexes::{EMPTY_HULL, EdgeIndex, HullIndex, HullVec, PointIndex, PointVec};
...
impl Hull {
    ...
    /// Checks if a point is currently on the hull (not erased)
    pub fn is_on_hull(&self, p: PointIndex) -> bool {
        if self.points.is_empty() {
            return false;
        }
        let h = self.points[p];
        ...
        // Check if the hull entry is still valid (not erased)
        self.data[h].left != EMPTY_HULL || self.data[h].right != EMPTY_HULL
    }
    ...
    pub fn insert_bare(&mut self, angle: f64, point: PointIndex, e: EdgeIndex) -> HullIndex {
