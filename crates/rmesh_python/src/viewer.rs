...
fn show_polygon2d(
    ...
    let options = rmesh_viewer::ViewerOptions {
        title: title.to_string(),
        width,
        height,
...
fn show_polylines_2d(
    ...
    let mut bb = rmesh::bounds::Bounds2::empty();
    ...
    if bb.is_empty() {
        bb = rmesh::bounds::Bounds2::new(Point2::new(-1.0, -1.0), Point2::new(1.0, 1.0));
    }

    let data = rmesh_viewer::View2DData {
        lines,
        fills: Vec::new(),
        bounds: bb,
    };
    ...
    py.detach(|| rmesh_viewer::show_2d_data(&data, options))
        .map_err(|e: anyhow::Error| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
    Ok(())
}

/// Register viewer functions into the Python module.
pub fn register_viewer(m: &Bound<'_, PyModule>) -> PyResult<()> {
